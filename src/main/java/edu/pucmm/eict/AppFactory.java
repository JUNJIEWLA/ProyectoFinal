...
public final class AppFactory {
    ...
    public static Javalin create(FormularioService formularioService, UserService userService, JwtService jwtService) {
        ...
        app.get("/api/formularios", ctx -> {
            ...
            DecodedJWT jwt = ctx.attribute("jwt");
            String role = jwt.getClaim("rol").asString();
            String requesterEmail = jwt.getClaim("email").asString();
            ...
            if (!canListUsuario(role, requesterEmail, usuarioRegistro)) {
                throw new ForbiddenResponse("No tienes permisos para listar otros usuarios");
            }
    ...
    private static boolean canListUsuario(String role, String requesterEmail, String usuarioRegistro) {
        if ("ADMIN".equals(role)) {
            return true;
        }
        return requesterEmail != null && requesterEmail.equalsIgnoreCase(usuarioRegistro);
    }
